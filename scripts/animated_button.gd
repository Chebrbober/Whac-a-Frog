...
func _ready() -> void:
	...
	button_up.connect(_button_release)

	mouse_entered.connect(_button_hover)
	mouse_exited.connect(_button_un_hover)

	stretch_mode = TextureButton.StretchMode.STRETCH_KEEP_CENTERED
...
func _button_release() -> void:
	if animation_tween:
		animation_tween.kill()
	animation_tween = create_tween().set_trans(trans_type).set_ease(ease_type)
	animation_tween.tween_property(self , "scale", Vector2.ONE, press_animation_duration_2)
